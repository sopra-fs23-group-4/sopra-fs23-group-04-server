...
public class GameServiceTest {
    ...
    void getScoreboard_validGameId_returnsScoreboard() {
        ...
        // Call the method
        List<ScoreboardEntryDTO> scoreboard = gameService.getScoreboard(gameId);

        // Verify results
        assertEquals(2, scoreboard.size());

        ScoreboardEntryDTO entry1 = scoreboard.get(0);
        assertEquals(user1, entry1.getUser());
        assertEquals(user1Score, entry1.getScore());

        ScoreboardEntryDTO entry2 = scoreboard.get(1);
    ...
    void constructor_initializesCorrectly() {
        ...
        GameService gameService = new GameService(gameRepository, userRepository, roundRepository, roundService, answerRepository);
